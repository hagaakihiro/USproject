...
x = MaxPooling3D((2, 2, 2), padding = 'same')(x) # 10 > 5

x = conv_block(x)
x = MaxPooling3D((2, 2, 2), padding = 'same')(x) # 5 > 3

x = conv_block(x)
x = conv_block(x)
x = MaxPooling3D((2, 2, 2), padding = 'same')(x) # 3 > 2

x = conv_block(x)
x = conv_block(x)
x = GlobalAveragePooling3D()(x)#(2, 2, 2), padding = 'same')(x) # 2 > 1
...
#this model
#Total params: 614,490
#Trainable params: 612,878
...
for itest in range(istep):
    ...
    for jj in range(1):
        ...
        opt = SGD(lr=0.05, decay=1e-2, momentum=0.9, nesterov=True)
