...
def conv_block(x, n_filter = 32, conv_size = (3, 3, 3)):
    x = Conv3D(filters = n_filter, kernel_size = conv_size, strides = (1, 1, 1),
...
def res_block(x, n_filter = 32, conv_size = (3, 3, 3)):
    y = Conv3D(filters = n_filter, kernel_size = conv_size, strides = (1, 1, 1),
               padding='same', kernel_initializer='he_normal',
               kernel_regularizer=l2(0.01))(x)
...
y = conv_block(input_img)
x = conv_block(y)
x = Concatenate(axis = -1)([x, y])
y = MaxPooling3D((2, 2, 2), padding = 'same')(x) # 10, 120, 120 > 5, 60, 60

x = res_block(y)
x = res_block(x)
x = Concatenate(axis = -1)([x, y])
y = MaxPooling3D((1, 2, 2), padding = 'same')(x) # 5, 60, 60 > 5, 30, 30

x = res_block(y)
x = res_block(x)
x = Concatenate(axis = -1)([x, y])
y = MaxPooling3D((2, 2, 2), padding = 'same')(x) # 5, 30, 30 > 3, 15, 15
...
x = Concatenate(axis = -1)([x, y])
x = BatchNormalization()(x)
x = MaxPooling3D((1, 2, 2), padding = 'same')(x) # 3, 15, 15 > 3, 8, 8

x = res_block(x)
x = AveragePooling3D((3, 2, 2), padding = 'valid')(x) # 3, 8, 8 > 1, 4, 4
