...
def conv_block(x, n_filter = 32, conv_size = (3, 3, 3)):
    x = Conv3D(filters = n_filter, kernel_size = conv_size,
               strides = (1, 1, 1),
               padding='same', kernel_initializer='he_normal',
               kernel_regularizer=l2(0.01))(x) # conv 1
    x = BatchNormalization()(x)
    x = ReLU()(x)
    x = Dropout(0.4)(x)
    return x

def res_block(x, y, n_filter = 32, conv_size = (3, 3, 3)):
    x = Conv3D(filters = n_filter, kernel_size = conv_size,
               strides = (1, 1, 1),
               padding='same', kernel_initializer='he_normal',
               kernel_regularizer=l2(0.01))(x) # conv 2
    z = Dropout(0.4)(x)
...
x, y = res_block(x, x)
x, y = MaxPooling3D((2, 2, 2), padding = 'same')(x),\
       MaxPooling3D((2, 2, 2), padding = 'same')(y)# 10, 120, 120 > 5, 60, 60
x, y = res_block(x, y)
x, y = res_block(x, y)
x, y = MaxPooling3D((2, 2, 2), padding = 'same')(x),\
       MaxPooling3D((2, 2, 2), padding = 'same')(y)# 5, 60, 60 > 3, 30, 30
x, y = res_block(x, y)
x, y = res_block(x, y)
x, y = MaxPooling3D((1, 2, 2), padding = 'same')(x),\
       MaxPooling3D((1, 2, 2), padding = 'same')(y) # 3, 30, 30 > 3, 15, 15
x, y = res_block(x, y)
x, y = res_block(x, y)
x, y = MaxPooling3D((1, 2, 2), padding = 'same')(x),\
       MaxPooling3D((1, 2, 2), padding = 'same')(y) # 3, 15, 15 > 3, 8, 8
x, y = res_block(x, y, n_filter = 64)
x, y = res_block(x, y, n_filter = 64)
x = Concatenate(axis = -1)([x, y])
x = GlobalAveragePooling3D()(x)

#x = Flatten()(x) # if GlobalAveragePooling3D
x = Dropout(0.4)(x)
x = Dense(64)(x)
x = BatchNormalization()(x)
x = ReLU()(x)
...
#no-Res model
#Total params: 222,818
#Trainable params: 222,754
#Non-trainable params: 64

#this model
#Total params: 652,706
#Trainable params: 651,170
#Non-trainable params: 1,536                                                                                                Non-trainable params: 1,280
###########################################################################追加部分
...
#        output_d = pd.DataFrame([[itest,jj,sum_acc,acc_train,acc_val,threshold_train,threshold_val,test_acc]])
        output_d = pd.DataFrame([[itest,jj,acc_train,acc_val,threshold_train,threshold_val,test_acc]])
        if itest == 0 and jj == 0:
